...
#define NUMDEV 30
#define NUMPLAYERS 6
#define MAX_PLAYERS_PER_DEVICE 2
...
typedef struct
{
	...
	uint8_t  num;
	uint8_t  player_num[2];
	uint8_t  player_index;		// per device player index
	uint8_t  num_players;
	uint8_t  has_map;
...
void reset_players()
{
	for (int i = 0; i < NUMDEV; i++)
	{
		input[i].map_shown = 0;
		input[i].num = 0;
		for(int j = 0; j < input[i].num_players; j++)
			input[i].player_num[j] = 0;
	}
...
static void input_cb(struct input_event *ev, struct input_absinfo *absinfo, int dev)
{
	...
	if (!input[dev].num)
	{
		...
		if (assign_btn)
		{
			for (uint8_t num = 1; num < NUMDEV + 1; num++)
			{
				...
				for (int i = 0; i < NUMDEV; i++)
				{
					if (input[i].quirk != QUIRK_TOUCHGUN)
					{
						...
						if (!((input[dev].quirk == QUIRK_PDSP || input[dev].quirk == QUIRK_MSSP) ^ (input[i].quirk == QUIRK_PDSP || input[i].quirk == QUIRK_MSSP)))
						{
							...
							for (int j = 0; j < input[i].num_players; j++)
								found |= (input[i].player_num[j] == num);
							if (found) break;
						}
					}
				}
				...
				if (!found)
				{
					input[dev].num_players = (input[dev].quirk == QUIRK_JAMMA) ? 2 : 1;
					for (int i = 0; i < input[dev].num_players; i++) {
						input[dev].player_num[i] = num;
						store_player(num, dev);
						printf("Device %s assigned to player %d\n", input[dev].id, num);
						num++;
					}
					input[dev].num = input[dev].player_num[input[dev].player_index];
					break;
				}
			}
		}
	...
	if (!input[dev].map_shown && input[dev].num)
	{
		...
		if (cfg.controller_info)
		{
			...
			{
				char players_str[MAX_PLAYERS_PER_DEVICE*2];
				char *s = players_str;

				for (int i = 0; i < input[dev].num_players; i++ )
				{
					*s++ = '0' + input[dev].player_num[i];
					*s++ = '&';
				}
				*--s = 0;
				map_joystick_show(input[dev].map, input[dev].mmap, players_str);
			}
		}
	}
...
{
	uint16_t key;
	uint16_t player; // player number counting from 0
	uint16_t btn;
} jamma2joy[] =
{
	{KEY_5,         0, 0x120}, // 1P coin
	{KEY_1,         0, 0x121}, // 1P start (shift key)
	{KEY_UP,        0, 0x122}, // 1P up
	{KEY_DOWN,      0, 0x123}, // 1P down
	{KEY_LEFT,      0, 0x124}, // 1P left
	{KEY_RIGHT,     0, 0x125}, // 1P right
	{KEY_LEFTCTRL,  0, 0x126}, // 1P 1
	{KEY_LEFTALT,   0, 0x127}, // 1P 2
	{KEY_SPACE,     0, 0x128}, // 1P 3
	{KEY_LEFTSHIFT, 0, 0x129}, // 1P 4
	{KEY_Z,         0, 0x12A}, // 1P 5
	{KEY_X,         0, 0x12B}, // 1P 6
	{KEY_C,         0, 0x12C}, // 1P 7
	{KEY_V,         0, 0x12D}, // 1P 8
	...
	{KEY_9,         0, 0x12E}, // Test
	{KEY_TAB,       0, 0x12F}, // Tab (shift + 1P right)
	{KEY_ENTER,     0, 0x130}, // Enter (shift + 1P left)
	// ~ Tidle supportted?
	{KEY_P,         0, 0x131}, // P (pause) (shift + 1P down)
	{KEY_F1,        0, 0x132}, // Service
	{KEY_F2,        0, 0x133}, // Test
	{KEY_F3,        0, 0x134}, // Tilt

	{KEY_6,         1, 0x120}, // 2P coin
	{KEY_2,         1, 0x121}, // 2P start
	{KEY_R,         1, 0x122}, // 2P up
	{KEY_F,         1, 0x123}, // 2P down
	{KEY_D,         1, 0x124}, // 2P left
	{KEY_G,         1, 0x125}, // 2P right
	{KEY_A,         1, 0x126}, // 2P 1
	{KEY_S,         1, 0x127}, // 2P 2
	{KEY_Q,         1, 0x128}, // 2P 3
	{KEY_W,         1, 0x129}, // 2P 4
								...
								if (input[dev].quirk == QUIRK_JAMMA && ev.type == EV_KEY)
								{
									for (uint32_t i = 0; i < sizeof(jamma2joy) / sizeof(jamma2joy[0]); i++)
									{
										if (jamma2joy[i].key == ev.code)
										{
											...
											input[dev].num = input[dev].player_num[jamma2joy[i].player];
											input[dev].player_index = jamma2joy[i].player;
